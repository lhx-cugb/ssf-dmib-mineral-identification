...
注意：v4版本为论文的主要版本
v4版本的v2版本中，训练错误率为60%，主要用于消融实验设计
...
candidate_pool = [[2, 1, 1], [1, 0, 1], [1, 1, 0], [2, 1, 0], [3, 1, 0], [2, 0, 1], [3, 0, 1], [1, 1, 1], [3, 1, 1],
                  [1, 0, -1], [2, 1, -1], [2, 0, -1], [3, 0, -1], [1, 1, -1], [3, 1, -1],
                  [1, -1, 0], [2, -1, 0], [3, -1, 0], [2, -1, 1], [1, -1, 1], [3, -1, 1],
                  [-1, 1, 0], [-1, 0, 1], [-1, 1, 1],
                  [2, -1, -1], [1, -1, -1], [3, -1, -1],
                  [-1, 0, -1], [-1, 1, -1],
                  [-1, -1, 0], [-1, -1, 1]]
...
# 构建一个硬度编号的字典
hardness_dict = {'Agate': [-1, -1, 0], 'Albite': [-1, -1, 0], 'Almandine': [-1, -1, 0],
                 'Anglesite': [-1, -1, 1], 'Azurite': [-1, -1, 1], 'Beryl': [-1, -1, 0],
                 'Cassiterite': [-1, -1, 0], 'Chalcopyrite': [-1, -1, 1], 'Cinnabar': [-1, -1, 1],
                 'Copper': [-1, -1, 1], 'Demantoid': [-1, -1, 0], 'Diopside': [-1, -1, 0],
                 'Elbaite': [-1, -1, 0], 'Epidote': [-1, -1, 1], 'Fluorite': [-1, -1, 0],
                 'Galena': [-1, -1, 1], 'Gold': [-1, -1, 1], 'Halite': [-1, -1, 0],
                 'Hematite': [-1, -1, 1], 'Magnetite': [-1, -1, 1], 'Malachite': [-1, -1, 1],
                 'Marcasite': [-1, -1, 1], 'Opal': [-1, -1, 0], 'Orpiment': [-1, -1, 1],
                 'Pyrite': [-1, -1, 1], 'Quartz': [-1, -1, 0], 'Rhodochrosite': [-1, -1, 0],
                 'Ruby': [-1, -1, 0], 'Sapphire': [-1, -1, 0], 'Schorl': [-1, -1, 1],
                 'Sphalerite': [-1, -1, 1], 'Stibnite': [-1, -1, 1], 'Sulphur': [-1, -1, 1],
                 'Topaz': [-1, -1, 0], 'Torbernite': [-1, -1, 1], 'Wulfenite': [-1, -1, 0]}
...
class multi_student_res(nn.Module):
    def __init__(self, num_classes=36, meta_dim=3):
        ...
        # 定义一个错误条件下的分类头--2mo
        self.classifier_2mo_12_error = nn.Linear(2048 + self.num_classes, num_classes)
        self.classifier_2mo_13_error = nn.Linear(2048 + self.num_classes, num_classes)
        self.classifier_2mo_23_error = nn.Linear(2048 + self.num_classes, num_classes)

        # 定义一个错误条件下的分类头--1mo--条痕
        self.classifier_1mo_1_error = nn.Linear(2048 + self.num_classes, num_classes)
        self.classifier_1mo_2_error = nn.Linear(2048 + self.num_classes, num_classes)
        self.classifier_1mo_3_error = nn.Linear(2048 + self.num_classes, num_classes)

        # 定义一个单模态新分类头--0mo
        self.classifier_0mo_error = nn.Linear(2048, num_classes)

        # 定义一个信息瓶颈层
        self.info_layer = InfoBottleneck(2048 + self.num_classes, 256)

        # 定义一个单模态使用的信息瓶颈层
        self.info_layer_0mo = InfoBottleneck(2048, 256)
    ...
    def get_multi_fc(self, x, hard, miss):
        ...
        if miss == 0:
            ...
            cls_info = self.classifier_2mo_12_error(fused_info)
            cls = self.classifier_2mo_12(fused)
        elif miss == 12:
            cls_info = self.classifier_2mo_13_error(fused_info)
            cls = self.classifier_2mo_13(fused)
        elif miss == 13:
            cls_info = self.classifier_2mo_23_error(fused_info)
            cls = self.classifier_2mo_23(fused)
        elif miss == 14:
            cls_info = self.classifier_1mo_1_error(fused_info)
            cls = self.classifier_1mo_1(fused)
        elif miss == 15:
            cls_info = self.classifier_1mo_2_error(fused_info)
            cls = self.classifier_1mo_2(fused)
        elif miss == 16:
            cls_info = self.classifier_1mo_3_error(fused_info)
            cls = self.classifier_1mo_3(fused)
        else:
            cls = self.classifier_3mo(fused)
    ...
    def forward(self, x, hard, miss):
        if miss == 1:  # 模态完全缺失的情况
            ...
            cls = self.get_single_fc(x)
            feas = self.get_img_features(x)
            cls_info = self.classifier_0mo_error(self.info_layer_0mo(feas))
            output = (cls, cls_info)
        else:
            output = self.get_single_fc(x)
        return output
...
# model_weight = "SSF_Net_v6_1mo_3_epoch_79_ckt.pth"
# model.load_state_dict(torch.load(model_weight, map_location=device)['network'], strict=False)
model.to(device)

model_name = "SSF_Net_v7_error_found_0mo_v4_v3_e60"
...
def evaluate(model, criterion):
    ...
    with torch.no_grad():
        ...
        for image, label, hardness_token in my_dataloader_val:
            ...
            pred, pred_info = model(image, hardness_token, 17)
...
if __name__ == '__main__':
    ...
    for epoch in range(epoches):
        ...
        for image, label, hardness_token in my_dataloader:
            ...
            student_output, student_output_info = model(image, corrupted_hardness_token, 17)
            student_target = torch.softmax(student_output, dim=1)
            # 计算硬标签损失
            hard_loss = criterion_cls(student_output_info, label)
            # 计算相关性损失
            p_origin = F.softmax(student_output, dim=-1)
            p_info = F.log_softmax(student_output_info, dim=-1)
