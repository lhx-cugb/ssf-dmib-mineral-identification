...
hardness_dict = {'Agate': [-1, 1, 0], 'Albite': [-1, 1, 0], 'Almandine': [-1, 1, 0],
                        ...
                        'Cassiterite': [-1, 1, 0], 'Chalcopyrite': [-1, 0, 1], 'Cinnabar': [-1, 0, 1],
                        'Copper': [-1, 0, 1], 'Demantoid': [-1, 1, 0], 'Diopside': [-1, 1, 0],
                        'Elbaite': [-1, 1, 0], 'Epidote': [-1, 1, 1], 'Fluorite': [-1, 1, 0],
                        'Galena': [-1, 0, 1], 'Gold': [-1, 0, 1], 'Halite': [-1, 1, 0],
                        'Hematite': [-1, 0, 1], 'Magnetite': [-1, 0, 1], 'Malachite': [-1, 1, 1],
                        'Marcasite': [-1, 0, 1], 'Opal': [-1, 1, 0], 'Orpiment': [-1, 1, 1],
                        'Pyrite': [-1, 0, 1], 'Quartz': [-1, 1, 0], 'Rhodochrosite': [-1, 1, 0],
...
model_weight = "SSF_Net_v6_2mo_23_epoch_49_ckt.pth"
model.load_state_dict(torch.load(model_weight, map_location=device)['network'], strict=False)
model.to(device)

model_name = "SSF_Net_v7_error_found_2mo_23_v4_v3_e60"
...
optimizer = SGD([
    ...
    {'params': model.classifier_2mo_23_error.parameters()},
...
def evaluate(model, criterion):
    ...
    with torch.no_grad():
        ...
        for image, label, hardness_token in my_dataloader_val:
            ...
            pred, pred_info = model(image, hardness_token, 13)
...
if __name__ == '__main__':
    ...
    for epoch in range(epoches):
        ...
        for image, label, hardness_token in my_dataloader:
            ...
            student_output, student_output_info = model(image, corrupted_hardness_token, 13)
